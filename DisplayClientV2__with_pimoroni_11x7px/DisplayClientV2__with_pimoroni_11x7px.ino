...
const byte* gear_images_array[] = {
  gear_empty_map,
  gear_r_map,
  gear_n_map, //gear_h_pattern_map,
  gear_1_map,
  gear_2_map,
  gear_3_map,
  gear_4_map,
...
#define DEVICE_UNIQUE_ID "6bceba9d-a5bb-42c1-b2ef-4b5971fe7e7d" //{"UniqueId":"6bceba9d-a5bb-42c1-b2ef-4b5971fe7e7d","Name":"DEVICE_UNIQUE_ID","Type":"uniqueid"}
...
// --------------------------------------------------------------------------------------------------------
// 0 disabled, 1 enabled
#define ENABLE_74HC595_GEAR_DISPLAY 1 //{"Group":"74HC595 Gear Display","Name":"ENABLE_74HC595_GEAR_DISPLAY","Title":"74HC595 gear display enabled","DefaultValue":"0","Type":"bool"}
#ifdef INCLUDE_74HC595_GEAR_DISPLAY
// Reverse led state
int RS_74HC595_INVERT = 1;
...
int gear_image_pimoroni_11x7px;
...
void RS_74HC595_SendData(byte data) {
	...
	// shift out the bits:
  shiftOut(RS_74HC595_DATAPIN, RS_74HC595_CLOCKPIN, LSBFIRST, data);
  shiftOut(RS_74HC595_DATAPIN, RS_74HC595_CLOCKPIN, LSBFIRST, B11111111);
...
/*gear_images_array[]
  ...
  // send data for Pimoroni 11x7px display
  // this is step 1...
  for (uint8_t x = 0; x < 7; x++) {
    for (uint8_t y = 0; y < 6; y++) {
      ledmatrix.drawPixel(x, y, gear_images_array[gear_image_pimoroni_11x7px][y * 7 + x] / 8); // transfer first half of the picture
    }
  }
  // and this is step 2
  for (uint8_t x = 8; x < 15; x++) {
    for (uint8_t y = 0; y < 5; y++) {
      ledmatrix.drawPixel(x, y, gear_images_array[gear_image_pimoroni_11x7px][ (7 * 6) + y * 7 + (x - 8)] / 8); // transfer second half of the picture
    }
  }
...
void RS_74HC595_SetChar(char c) {
	byte data = 0;
	if (c == ' ') {
    gear_image_pimoroni_11x7px = 0;
		data = RS_74HC595_ClearDigit;
	}
	else if (c == 'R') {
    gear_image_pimoroni_11x7px = 1;
		data = RS_74HC595_ReverseDigit;
	}
	else if (c == 'N') {
    gear_image_pimoroni_11x7px = 2;
		data = RS_74HC595_NeutralDigit;
	}
	else {
		int value = c - '0';
		if (value >= 0 && value < 10) {
      gear_image_pimoroni_11x7px = value + 2;
